...
//     setTimeout(() => setIsLeftClicked(false), 300); // Reset after 200ms
//   };

//   const handleRightClick = () => {
//     setIsRightClicked(true);
//     setTimeout(() => setIsRightClicked(false), 300); // Reset after 200ms
//   };
...
//         </div>
//       </div>
//       <div className="flex space-x-4 lg:w-[85%] xl:w-[88%]">
//         <img src={square} alt="" className="self-start"/>
//         {/* Images would go here */}
//         <img src={image} alt="" className="w-1/3 h-44 rounded-md object-cover" />
//         <img src={image} alt="" className="w-1/3 h-44 rounded-md object-cover" />
//         <img src={image} alt="" className="w-1/3 h-44 rounded-md object-cover" />
//       </div>
//     </div>
...
const GalleryWidget = () => {
  ...
  const handleLeftClick = () => {
    setIsLeftClicked(true);
    setTimeout(() => setIsLeftClicked(false), 300); // Reset after 300ms
  };

  const handleRightClick = () => {
    setIsRightClicked(true);
    setTimeout(() => setIsRightClicked(false), 300); // Reset after 300ms
  };
      ...
      <div className="flex space-x-4 lg:w-[85%] xl:w-[98%] relative">
        ...
        {[image, image, image].map((img, index) => (
          <div key={index} className="relative w-1/3 h-44 group overflow-hidden">
            ...
            {/* Angle Image (hidden by default, zooms out on hover) */}
            <img
              src={angle}
              alt="angle overlay"
